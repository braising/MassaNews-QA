...
namespace MassaNews.Portal.Controllers
{
  public class HomeController : BaseController
  {
    ...
    private HomeIndex GetHomeIndexModel(int locationId)
    {
      ...
      #region Cotação Economia

      List<string> lstMoedas = new List<string>
      {
        "USD",
        "EUR",
        "GBP",
      ...
      List<CotacaoEconomiaViewModel> lstCotacaoEconomia = new List<CotacaoEconomiaViewModel>();

      foreach (var moeda in lstMoedas)
      {
        Entities.Tables.CotacaoEconomia objCotacaoEconomia = GetCotacaoByMoeda(moeda);

        CotacaoEconomiaViewModel objCotacao = new CotacaoEconomiaViewModel
        {
          Simbolo = objCotacaoEconomia.Simbolo,
          Moeda = objCotacaoEconomia.Moeda,
          TaxaVenda = decimal.Parse(objCotacaoEconomia.TaxaVenda),
          Varicao = objCotacaoEconomia.Variacao
        };

        lstCotacaoEconomia.Add(objCotacao);
      }

      objModel.CotacoesEconomia = lstCotacaoEconomia;
      #endregion
    ...
    private Entities.Tables.CotacaoEconomia GetCotacaoByMoeda(string moeda)
    {
      return Db.CotacaoEconomia.Where(c => c.Simbolo == moeda).OrderByDescending(c => c.DataCadastro).FirstOrDefault();
    }
